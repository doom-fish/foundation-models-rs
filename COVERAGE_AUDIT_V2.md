...
**V2 validation notes (re-audit of v1):** All 114 EXEMPT entries from v1 have been re-validated against `arm64e-apple-macos.swiftinterface`. 87 exemptions are justified by one of: (1) Swift @_functionBuilder/@attached attributes for DSL machinery, (2) Standard-library protocol conformance helpers (Equatable, Hashable, Collection, AsyncSequence), or (3) Extensions on Swift/Foundation stdlib types. The remaining 27 exemptions are builder overloads and conformance boilerplate that the crate appropriately excludes from Rust via idiomatic trait patterns rather than 1:1 FFI wrappers. All exemptions maintain editorial integrity per v1 methodology.
...
| `SystemLanguageModel.Adapter.AssetError.Context` | Struct | `FoundationModels.swiftinterface:L686` | `adapter error context extraction in FFI` |
| `SystemLanguageModel.Adapter.AssetError.Context.debugDescription` | Var | `FoundationModels.swiftinterface:L687` | `adapter error context extraction in FFI` |
| `SystemLanguageModel.Adapter.AssetError.Context.init(debugDescription: Swift.String)` | Init | `FoundationModels.swiftinterface:L688` | `adapter error context extraction in FFI` |
| `SystemLanguageModel.Adapter.AssetError.recoverySuggestion` | Var | `FoundationModels.swiftinterface:L699` | `mapError() extracts recovery suggestion in assetErrorPayload()` |
...
| --- | --- | --- | --- | --- |
| `SystemLanguageModel.Adapter.isCompatible(_ assetPack: BackgroundAssets.AssetPack)` | Func | `FoundationModels.swiftinterface:L673` | External framework dependency | `BackgroundAssets.AssetPack` not wrapped |
